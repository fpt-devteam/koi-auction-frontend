    ...
    const initializeConnection = async () => {
      ...
      try {
        ...
        newConnection.on(signalRMess.RECEIVE_START_AUCTION_LOT, () => {
          message.success(
            "The auction has started. Please place your bids now!",
            TIME_MESS
          );
        });
        ...
        newConnection.on(
          signalRMess.RECEIVE_PREDICT_END_TIME,
          (predictEndTime) => {
        ...
        newConnection.on(signalRMess.RECEIVE_END_AUCTION_LOT, (winner) => {
          ...
          const mess =
            winner == null
              ...
              : `Auction has ended. Winner is ${winner.bidderId} with bid ${winner.bidAmount}`;
          message.success(mess, TIME_MESS);
        });

        newConnection.on(
          signalRMess.RECEIVE_EXCEPTION_MESSAGE,
          (exceptionMessage) => {
          ...
          Modal.success({
            ...
            content: (
              <div>
                ...
                <p>
                  Expire time: <strong>{formattedExpTime}</strong>
                </p>
                <p>
                  Final Price: <strong>{finalPrice} VND</strong>
                </p>
              </div>
            ),
            onOk() {},
