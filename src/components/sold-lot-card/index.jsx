    ...
    <>
      <Card
        ...
        title={
          <div className="lot-card-title">
            <span style={{ color: "#838181" }}>
              #{soldLot?.lotDto?.lotId} -{" "}
            </span>
          ...
          <>
            ...
            >
              ...
              <Text className="lot-detail-item-content-money">
                {soldLot
                  ? formartMoney(
                      soldLot?.finalPrice - (soldLot?.auctionDeposit || 0)
                    )
                  : "..."}{" "}
                VND
              </Text>
            ...
            >
              ...
              <Button size="large" key="view-lot" onClick={toggleLotInfoModal}>
                View Lot
              </Button>
            </div>
          </>,
      ...
      >
          ...
          <Col span={12} style={{}}>
            <div className="lot-details">
              <div className="lot-detail-item">
                ...
                <Text className="lot-detail-item-content">
                  {soldLot?.lotDto?.koiFishDto?.variety}
                </Text>
              </div>
              <div className="lot-detail-item">
                <Text className="lot-detail-item-title">By: </Text>
                <Text className="lot-detail-item-content">
                  {soldLot?.breederDetailDto?.breederId}
                </Text>
              </div>
              <div className="lot-detail-item">
                <Text className="lot-detail-item-title">Farm Name: </Text>
                <Text className="lot-detail-item-content">
                  {soldLot?.breederDetailDto?.farmName}
                </Text>
              </div>
            </div>
          </Col>
        ...
        <Modal
          ...
          style={{ top: 0 }}
        >
          <LotInfoModal lotInfoData={soldLot} refresh={refresh} />
        </Modal>
        <Modal
...
const LotInfoModal = ({ lotInfoData }) => {
  const tableDataSource = [
    ...
    },
    {
      ...
      value: (
        <div className="info-content">
          {lotInfoData?.lotDto?.koiFishDto?.variety}
        </div>
      ),
    },
    {
      ...
      label: "Size",
      value: (
        <div className="info-content">{`${lotInfoData?.lotDto?.koiFishDto?.sizeCm} cm`}</div>
      ),
    },
    {
      key: "yearOfBirth",
      label: "Year of Birth",
      value: (
        <div className="info-content">{`${lotInfoData?.lotDto?.koiFishDto?.yearOfBirth}`}</div>
      ),
    },
    {
      key: "sex",
      ...
      value: (
        <div className="info-content">
          {lotInfoData?.lotDto?.koiFishDto?.sex ? "Male" : "Female"}
        </div>
      ),
    },
    {
      ...
      value: (
        <div className="info-content">
          {lotInfoData?.breederDetailDto?.farmName}
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      value: (
        <div className="info-content">
          {lotInfoData?.lotStatusDto?.lotStatusName}
        </div>
      ),
    },
  ];
...
const OrderInfoModal = ({
  ...
  const handleCheckExpireTime = () => {
    ...
    return dayjs(soldlotInfoData?.expTime).isAfter(
    ...
    try {
      ...
      await Promise.all([
        ...
        paymentApi.post(`payout`, {
          ...
          Description: `Payout for lot ${
            soldlotInfoData?.lotDto?.lotId
          }, total ${soldlotInfoData?.finalPrice} but 90% for breeder ${
            soldlotInfoData?.breederDetailDto?.breederId
          } is ${0.9 * soldlotInfoData?.finalPrice}`,
        }),
      ]).then(([response, paymentResponse]) => {
  ...
  const handlePayment = async () => {
    ...
    try {
      onCancel();
      await Promise.all([
        await paymentApi.post("/payment", {
          Amount: soldlotInfoData.finalPrice - soldlotInfoData.auctionDeposit,
          SoldLotId: soldlotInfoData.lotId,
          Description: `Payment for lot ${soldlotInfoData?.lotDto?.lotId} - ${
            soldlotInfoData.finalPrice - soldlotInfoData.auctionDeposit
          } VND`,
        }),
        await lotApi.put(`lots/${soldlotInfoData?.lotDto?.lotId}/status`, {
        ...
        if (paymentResponse.data) {
          ...
          message.error({
            content: `${paymentResponse.data?.message}`,
            key: "payment",
          });
        }
      });
      ...
      <div style={{ padding: "36px", paddingTop: "0px" }}>
        <div style={{ paddingTop: "24px" }}>
          <Row gutter={16} style={{ alignItems: "flex-end" }}>
            <Col span={15}>
              <Card
                ...
                title={<p className="inner-card-title">Delivery Infomation</p>}
              >
            ...
            <Col span={9}>
              <Card
                className="inner-card"
                title={<p className="inner-card-title">Status</p>}
              >
                <div className="info-container">
                  <span className="info-label">Status: </span>
                  <span className="info-content">
                    {soldlotInfoData
                      ? soldlotInfoData?.lotStatusDto?.lotStatusName
                      : "N/A"}
                  </span>
                </div>
                <div className="info-container">
                  ...
                  <span className="info-content">
                    {soldlotInfoData
                      ? formatDateTime(soldlotInfoData?.updatedLot)
                      : "...."}
                  </span>
                </div>
              </Card>
          ...
          <Card className="inner-card">
            <div>
              <Row>
                <Col className="info-field" span={6}>
                  <Image
                    src={
                      soldlotInfoData?.lotDto?.koiFishDto?.koiMedia?.[0]
                        ?.filePath
                    }
                    width={"100%"}
                    alt="Koi Fish"
                ...
                <Col className="info-field" span={18}>
                  ...
                  <Row>
                    <Col span={14}>
                      <div className="info-container">
                        ...
                        <span className="info-content">
                          {soldlotInfoData?.lotDto?.koiFishDto?.variety ||
                            "..."}
                        </span>
                      </div>
                      <div className="info-container">
                        ...
                        <span className="info-content">
                          {soldlotInfoData?.breederDetailDto?.farmName ||
                            "Unknown"}
                        </span>
                      </div>
                      <div className="info-container">
                        ...
                        <span className="info-content">
                          {soldlotInfoData?.breederDetailDto?.farmAddress ||
                            "Unknown"}
                        </span>
                      </div>
                    </Col>
            ...
            <div style={{ marginTop: "30px" }}>
              <Row>
                <Col span={8}>
                  {soldlotInfoData?.lotStatusDto?.lotStatusId ==
                    TO_PAY_STATUS_ID &&
                    handleCheckExpireTime() && (
                      <>
                ...
                <Col span={16}>
                  <Row>
                    ...
                    <Col className="info-payment-item-content" span={6}>
                      <span>
                        {soldlotInfoData
                          ? formartMoney(soldlotInfoData?.finalPrice)
                          : "..."}{" "}
                        VND
                      </span>
                    </Col>
                  ...
                  <Row>
                    ...
                    <Col className="info-payment-item-content" span={6}>
                      <span>
                        {formartMoney(soldlotInfoData?.auctionDeposit || 0)} VND
                      </span>
                    </Col>
                  </Row>
                  <Row>
                    ...
                    <Col className="info-payment-item-content-money" span={6}>
                      <span>
                        {soldlotInfoData
                          ? formartMoney(
                              soldlotInfoData?.finalPrice -
                                (soldlotInfoData?.auctionDeposit || 0)
                            )
                          : "..."}{" "}
                        VND
                      </span>
                    ...
                    <Col className="info-payment-item-content" span={6}>
                      <span>
                        {soldlotInfoData?.lotStatusDto?.lotStatusId > 6
                          ? soldlotInfoData?.lotStatusDto?.lotStatusId != 11
                            ? "Success"
                            : "Fail"
                          : "Pending"}
                      </span>
        ...
        >
          {user?.UserRoleId == 1 &&
            soldlotInfoData?.lotStatusDto?.lotStatusId == TO_PAY_STATUS_ID &&
            handleCheckExpireTime() && (
              <Button
                type="primary"
                size="large"
          ...
          {user?.UserRoleId > 2 && tabData.lotStatusId == 7 && (
            <>
              <Button type="primary" size="large" onClick={handleLotDelivery}>
                Delivery
              </Button>
              <Button
                type="primary"
          ...
          {user?.UserRoleId > 2 && tabData.lotStatusId == 8 && (
            <>
              <Button type="primary" size="large" onClick={handleLotComplete}>
                Complete
              </Button>
              <Button
                type="primary"
